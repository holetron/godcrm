...
// ADR-0068 WP-C: structured reply-to. Server fetches the source message by
// `message_id` and validates `fragment`/`range` against the canonical content —
// the client never trusts (or sends) sender/content for the parent.
// `fragment` = verbatim substring of source for partial-quote.
// `range` = `[startOffset, endOffset]` char positions when fragment is implicit.
...
export interface AIChatContextValue extends AIChatState {
  ...
  sendMessage: (content: string, attachments?: File[], modelId?: number, mentions?: Mention[], agentMode?: boolean, systemPromptPrefix?: string, subAgentRowIds?: number[], rowAttachments?: Array<{ id: string; name: string; type: string; size: number; rowReference: { table_id: number; row_id: number; table_name: string; table_icon?: string; row_title?: string } }>, convBoundRow?: { table_id: number; row_id: number }, replyTo?: ReplyTo) => Promise<void>;
  clearMessages: () => void;
  ...
  setIsAgentProcessing: (processing: boolean) => void; // Direct setter for syncing backend polling → context
  ...
  setProcessingStartedAt: (ts: number | null) => void; // For syncing processing start time from backend
  ...
  // External row attachment — called from UniversalTable etc.
  attachRowToChat: (row: { table_id: number; row_id: number; table_name?: string; table_icon?: string; row_title?: string }) => void;
  attachRowToMessage: (row: { table_id: number; row_id: number; table_name?: string; table_icon?: string; row_title?: string }) => void;
  pendingBoundRow: { table_id: number; row_id: number; table_name?: string; table_icon?: string; row_title?: string } | null;
  pendingMessageRow: { table_id: number; row_id: number; table_name?: string; table_icon?: string; row_title?: string } | null;
  clearPendingBoundRow: () => void;
  clearPendingMessageRow: () => void;

...
export function TelegramChatLayout({ className }: TelegramChatLayoutProps) {
  ...
  // ADR-0068 WP-E — pin/unpin mutator scoped to the current conversation. The
  // BE enforces visibility (group/agent: any participant; DM: owner) and
  // returns 403/409 which our hook turns into silent-rollback / cap-strip UX.
  const pinMutation = usePinMessage(currentConversationId);

  // ADR-0068 WP-E — UI gate that mirrors the BE rule (pinController.checkPinPermission).
  // DM (`conversation.type === 'direct'`) is owner-only; everything else (group/agent/channel)
  // any participant. We default to OFF until convMeta loads to avoid a brief
  // "Закрепить" flash for unauthorized users on first paint.
  const currentUserIdNum = authUser?.id ? Number(authUser.id) : undefined;
  const canPin = useMemo(() => {
    if (!convMeta || currentUserIdNum == null) return false;
    if (convMeta.type === 'direct') {
      return Number(convMeta.created_by) === currentUserIdNum;
    }
    return true;
  }, [convMeta, currentUserIdNum]);

  // ADR-0068 WP-E — cap surface: when usePinMessage flips `capReached` true we
  // surface a one-shot toast and immediately clear the notice so the next pin
  // attempt produces its own notice.
  useEffect(() => {
    if (pinMutation.capReached) {
      showToast('Достигнут лимит 50 закреплённых сообщений', 'error');
  ...
  // Listen for navigate-to-chat-message events (from forwarded message links)
  useEffect(() => {
  ...
  const handleSendMessage = async (
    ...
    await sendMessage(content, attachments, undefined, formattedMentions, undefined, undefined, subAgentIds, undefined, undefined, replyTo);
    ...
    <div className={cn(
      ...
      <div className={cn(
        ...
        <ChatConversationView
          ...
          onPinMessage={canPin ? pinMutation.pin : undefined}
          onUnpinMessage={canPin ? pinMutation.unpin : undefined}
          pinCapReached={pinMutation.capReached}
          onClearPinCapNotice={pinMutation.clearCapNotice}
        />

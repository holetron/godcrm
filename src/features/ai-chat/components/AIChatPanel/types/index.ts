...
export type { ChatMessage, ChatAttachment, ToolResult, AIAgent, MessageContentType, WidgetEmbedConfig, WidgetEmbedAttachment } from '../../../types';
// ADR-0031 WP-20+21 (T-141238): runtime guard for narrowing chat attachments
// to widget_embed shape — re-exported here for components that import the
// chat type bundle from this module (e.g. ChatTurn/TurnBody).
export { isWidgetEmbedAttachment } from '../../../types';

// Alias types for backward compatibility
export type { AIAgent as AIAgentType } from '../../../types';

// ADR-0068 WP-C: structured reply-to carried in composer state and POST body.
// Server fetches the source by `message_id` and validates `fragment`/`range`
// against the canonical content — client never trusts sender/content of parent.
export interface ReplyTo {
  /** Source message id (in /chat/conversations/:id/messages). */
  message_id: number;
  /** Verbatim substring of source content for partial-quote. */
  fragment?: string;
  /** `[startOffset, endOffset]` char positions when fragment is implicit. */
  range?: [number, number];
  /** UI-only preview fields populated by the bubble Quote action; ignored by POST.
   *  Used by composer chip + outgoing bubble chip until server enrichment lands. */
  _displaySender?: string;
...
export type SettingsTab = 'ai' | 'people' | 'widget' | 'notifications';
...
export interface TicketsSourceConfig {
  ...
  tableName: string;
  tableIcon?: string;
  /** Per-row emoji column (e.g. ticket-type icon). Falls back to `tableIcon`. */
  iconColumn?: string;
  displayColumn?: string;
...
// Favorite-tables config (chat attach popup) — stored on _spaces.favorites_config JSONB
export interface FavoriteTable {
  tableId: number;
  tableName: string;
  tableIcon?: string;
  // Optional column mapping (used by documents slot to map title/preview/etc.)
  iconColumn?: string;
  displayColumn?: string;
  descriptionColumn?: string;
  statusColumn?: string;
...
export interface FavoritesConfig {
  documents?: FavoriteTable | null;
  custom?: FavoriteTable[];
}

export type AttachTabId = 'files' | 'tickets' | 'documents' | 'other' | `favorite:${number}`;
...
export interface ChatState {
  ...
  favoritesConfig: FavoritesConfig | undefined;
  ...
  // ADR-0068 WP-C: composer reply-to (set by bubble Quote action; cleared after send)
...
export interface ChatActions {
  ...
  setFavoritesConfig: Dispatch<SetStateAction<FavoritesConfig | undefined>>;
  setShowFilePicker: Dispatch<SetStateAction<boolean>>;
  setAttachTab: Dispatch<SetStateAction<AttachTabId>>;
  ...
  setFavoriteWidth: Dispatch<SetStateAction<number>>;
  setIsResizingWidth: Dispatch<SetStateAction<boolean>>;
  setIsGlued: Dispatch<SetStateAction<boolean>>;
  ...
  // ADR-0068 WP-C: composer reply-to setter
  setReplyTo: Dispatch<SetStateAction<ReplyTo | null>>;

  // Inbox sort & unread filter
  inboxSortBy: 'created' | 'updated';
  setInboxSortBy: Dispatch<SetStateAction<'created' | 'updated'>>;
  inboxSortDir: 'asc' | 'desc';
  setInboxSortDir: Dispatch<SetStateAction<'asc' | 'desc'>>;
  inboxUnreadOnly: boolean;
  setInboxUnreadOnly: Dispatch<SetStateAction<boolean>>;
  inboxUserFilter: string[];
  setInboxUserFilter: Dispatch<SetStateAction<string[]>>;
  inboxParticipantMode: 'any' | 'all';
  setInboxParticipantMode: Dispatch<SetStateAction<'any' | 'all'>>;
}
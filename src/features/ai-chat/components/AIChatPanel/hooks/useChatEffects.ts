...
export function useChatEffects({
  ...
  useEffect(() => {
    ...
    const handleViewportResize = () => {
      ...
      rafId = requestAnimationFrame(() => {
        const rawHeight = window.innerHeight - viewport.height;
        // iOS Safari fires visualViewport.resize on URL-bar collapse/expand
        // during momentum scroll (rawHeight bounces 0↔~60). Treat anything
        // below the keyboard threshold as zero so the panel's `bottom` style
        // does not churn and re-render the chat list mid-scroll.
        const keyboardHeight = rawHeight > 50 ? rawHeight : 0;
        actions.setMobileKeyboardHeight(keyboardHeight);

        // Scroll only once when keyboard first opens (not on every resize event)
        // Use container.scrollTop instead of scrollIntoView — the latter scrolls ALL
        // ancestor scrollable containers, causing the main layout to jump.
  ...
  }, [state.isMobile, actions, refs.messagesContainerRef]);

...
export function useChatMutations(params: UseChatMutationsParams) {
  ...
  // Helper: parse settings safely (may be string or object)
  const parseSettings = useCallback((raw: unknown): Record<string, unknown> => {
    if (!raw) return {};
  ...
  const saveDefaultAgent = useCallback(async (agentId: number | null) => {
    ...
    try {
      const currentSettings = parseSettings(currentSpace?.settings);
  ...
  }, [currentSpace, parseSettings, setDefaultAgentId, setIsSavingDefaultAgent, setCurrentSpaceInStore, updateSpaceInStore, queryClient]);
  ...
  }, [currentSpace, parseSettings, setQuickEmojis, setIsSavingEmojis, setCurrentSpaceInStore, updateSpaceInStore, queryClient]);
  ...
  const sendUserMessageMutation = useMutation({
    mutationFn: async ({ conversationId, content, agentMode: mode, thinking, mentions, attachments: msgAttachments, replyTo }: {

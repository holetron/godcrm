...
describe('TicketsPanel', () => {
  ...
  describe('when ticketsSource is configured', () => {
    ...
    it('should show type icon or hash in ticket icon badges', () => {
      render(<TicketsPanel {...defaultProps} />);

      // Component renders typeEmoji || '#' in the icon badge (no type field in test data → '#')
      const hashIcons = screen.getAllByText('#');
      expect(hashIcons.length).toBe(3);
    });

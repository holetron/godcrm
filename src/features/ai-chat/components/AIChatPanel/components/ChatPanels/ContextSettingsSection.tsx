...
export const ContextSettingsSection: React.FC<ContextSettingsSectionProps> = ({
  ...
  // Section collapse state (only Context Depth is collapsible — Auto-Summary
  // and Vector Memory expose a top-level checkbox instead of a chevron).
  ...
  return (
    <div className="space-y-1">
      ...
      {/* ─── Section: Auto-Summary (top-level checkbox) ─────────────── */}
      <div className="border-t border-[var(--border-secondary)] pt-2">
        <label
          className={cn(
            'w-full flex items-start gap-2 py-2 cursor-pointer',
            disabled && 'opacity-50 cursor-not-allowed'
          )}
        >
          <input
            ...
            onChange={(e) => updateAutoSummary({ enabled: e.target.checked })}
            disabled={disabled}
            className="mt-0.5 w-3.5 h-3.5 rounded border-[var(--border-primary)] text-purple-500 focus:ring-purple-500/30"
          />
          <div className="w-5 h-5 flex items-center justify-center text-[var(--text-tertiary)]">
      ...
      {/* ─── Section: Vector Memory (top-level checkbox) ────────────── */}
      <div className="border-t border-[var(--border-secondary)] pt-2">
        <label
          className={cn(
            'w-full flex items-start gap-2 py-2 cursor-pointer',
        ...
        >
          <input
            ...
            onChange={(e) => updateVectorSearch({ enabled: e.target.checked })}
            disabled={disabled}
            className="mt-0.5 w-3.5 h-3.5 rounded border-[var(--border-primary)] text-purple-500 focus:ring-purple-500/30"
          />
          <div className="w-5 h-5 flex items-center justify-center text-[var(--text-tertiary)]">

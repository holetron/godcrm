...
import { TerminalPanel, CommandApprovalBar, usePendingAgentApproval } from '@/features/terminal';
...
export function ChatInputArea(props: ChatInputAreaProps) {
  ...
  const approval = usePendingAgentApproval(panelMode !== 'fullscreen');

  if (panelMode === 'fullscreen') return null;

  return (
    <>
      {/* Pending command approval — inline strip in the same slot as polling errors */}
      {approval.pending && (
        <CommandApprovalBar
          pending={approval.pending}
          onApprove={approval.approve}
          onReject={approval.reject}
          onAlwaysAllow={approval.alwaysAllow}
          isApproving={approval.isApproving}
          isRejecting={approval.isRejecting}
          isPolicyWriting={approval.isPolicyWriting}
          policyError={approval.policyError}
        />
      )}
      {/* Attachments Preview */}

...
export const EditProjectModal = ({ open, onOpenChange, onDeleteClick, project }: EditProjectModalProps) => {
  ...
  const [accessControl, setAccessControl] = useState<AccessControlConfig>(
    project.access_control || DEFAULT_ACCESS_CONTROL
  );
  ...
  useEffect(() => {
    ...
    setAccessControl(project.access_control || DEFAULT_ACCESS_CONTROL);
    setError('');
    setActiveTab('display');
  }, [project.id, project.name, project.description, project.icon, project.access_control]);
      ...
      <div className="space-y-4">
        ...
        <div className="min-h-[300px]">
          ...
          {activeTab === 'display' && (
            <div className="space-y-4">
              <div>
                <label htmlFor="project-name" className="mb-2 block text-sm font-medium text-[var(--text-primary)]">
                    ...
                    {accessControl.usersTableId && tableColumns.length > 0 && (
                      <>
                        ...
                        <div className="p-3 rounded-lg bg-primary-500/10 border border-primary-500/30 text-sm text-primary-400">
                          <strong>ℹ️ Наследование:</strong> Если не настроено, проект наследует права от пространства.
                        </div>
                      </>
                    )}
                  </div>
        ...
        <div className="flex items-center justify-between gap-3 pt-4 border-t border-[var(--border-primary)]">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="primary" onClick={handleSubmit} disabled={updateProject.isPending}>

...
export function formatShortDate(value: unknown): string {
  ...
  try {
    ...
    const months = ['янв', 'фев', 'мар', 'апр', 'май', 'июн', 'июл', 'ав��', 'сен', 'окт', 'ноя', 'дек'];
...
// Resolve card title: try configured column first, then common name fields
export function resolveCardTitle(data: Record<string, unknown> | undefined, cardTitleColumn: string): string {
  if (!data) return 'Untitled';
  const primary = data[cardTitleColumn];
  if (primary && String(primary).trim()) return String(primary);
  for (const field of ['name', 'title', 'what', 'subject', 'label']) {

...
export const ColorPicker = ({ value, onChange, compact = false }: ColorPickerProps) => {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div
          className="absolute top-full left-0 mt-1 z-[9999] bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-lg shadow-lg p-2"
          ...
          <div className="grid gap-1" style={{ gridTemplateColumns: 'repeat(6, 1fr)' }}>
            {COLUMN_COLORS.map((c, i) => (
              <button
                ...
                title={c || t('tables.create.noColor')}
              />
            ))}
          </div>
        </div>

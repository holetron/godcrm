...
import { Inbox, Loader2, User, Users, ChevronRight, Bot, Search, X, Calendar, SlidersHorizontal } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          filteredConversations.map(conv => {
            ...
            return (
              <button
                key={`${conv._source || 'u'}-${conv.id}`}
                onClick={() => handleConversationClick(conv)}
                className={cn(
                  "w-full flex items-center gap-3 px-3 py-2.5 hover:bg-[var(--bg-tertiary)] transition-colors text-left border-b border-[var(--border-secondary)]/50",
              ...
              >
                  ...
                  <div className="text-[10px] text-[var(--text-tertiary)] truncate">
                    {isAi ? (
                      <>
                        ...
                        {conv.bound_row_id && <span> · 🔗 #{conv.bound_row_id}</span>}
                      </>
                    ) : (
                      <>
                        {conv.type === 'direct' ? 'Личный чат' : `${conv.participants?.length || 0} участников`}
                      </>
                    )}
                    {' · '}
                ...
                {conv.unread_count > 0 && (
                  <ChevronRight className="w-4 h-4 text-[var(--color-primary-500)] flex-shrink-0" />
                )}
              </button>
            );
          })

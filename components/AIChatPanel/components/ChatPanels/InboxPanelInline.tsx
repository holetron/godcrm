...
import { X, Search, Loader2, Inbox, Filter, User, Users, ChevronRight, Link2, Pencil } from 'lucide-react';
      ...
      {showInboxFilters && (
        <div className="px-3 py-2 border-b border-[var(--border-secondary)] space-y-2 bg-[var(--bg-secondary)]">
          <div>
            ...
            <select value={inboxAgentFilter} onChange={(e) => setInboxAgentFilter(e.target.value)}
              ...
              {inboxAgentOptions.map(agent => (<option key={agent.id} value={agent.id}>{agent.icon || '🤖'} {agent.name}</option>))}
            </select>
          </div>

...
import { X, Search, Star, Loader2, Brain, Bot, Plus, MessageSquare } from 'lucide-react';
    ...
    <div className="flex flex-col h-full">
      <div className="px-2 py-1.5 border-b border-[var(--border-secondary)] flex items-center gap-1.5">
        <div className="flex-1 relative">
          ...
          <input type="text" value={d.agentsSearch}
        ...
        <button onClick={() => d.setShowFavoriteAgents(!d.showFavoriteAgents)}
          className={cn("p-1.5 rounded-lg border transition-colors",
            d.showFavoriteAgents ? "bg-yellow-500/20 text-yellow-500 border-yellow-500/30" : "text-[var(--text-tertiary)] border-[var(--border-secondary)] hover:bg-[var(--bg-tertiary)] hover:text-[var(--text-primary)]"
          )} title={d.showFavoriteAgents ? "Показать всех" : "Только избранные"}>
          <Star className={cn("w-3.5 h-3.5", d.showFavoriteAgents && "fill-current")} />
        </button>
        <button onClick={d.handleVectorSearch} disabled={d.isVectorSearching || !d.agentsSearch}
          className={cn("p-1.5 rounded-lg border transition-colors",
            d.vectorSearchResults ? "bg-green-500/20 text-green-400 border-green-500/30" : "text-[var(--text-tertiary)] border-[var(--border-secondary)] hover:bg-purple-500/10 hover:text-purple-400 hover:border-purple-500/30",
          ...
          d.filteredAgents.map(agent => {
            ...
            return (
              <div key={agent.id} className="border-b border-[var(--border-secondary)] last:border-b-0">
                <div className={cn("flex items-center gap-2 px-3 py-2 transition-colors group",
                  d.currentAgent?.id === agent.id ? "bg-[var(--color-primary-500)]/10" : "hover:bg-[var(--bg-tertiary)]")}>
                  <button onClick={(e) => { e.stopPropagation(); d.setFavoriteAgents(prev => isFavorite ? prev.filter(id => id !== agent.id) : [...prev, agent.id]); }}
                    className={cn("p-1 rounded border transition-colors",
                      isFavorite ? "text-yellow-500 border-yellow-500/30 bg-yellow-500/10" : "text-[var(--text-tertiary)] border-[var(--border-secondary)] hover:text-yellow-500 hover:border-yellow-500/30"
                    )} title={isFavorite ? "Убрать из избранного" : "В избранное"}>
                    <Star className={cn("w-3.5 h-3.5", isFavorite && "fill-current")} />
                  </button>
                  <button onClick={() => d.handleAgentSelect(agent)} className="flex items-center gap-2 flex-1 min-w-0 text-left">
                    <div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-base flex-shrink-0",
                      d.currentAgent?.id === agent.id ? "bg-[var(--color-primary-500)]/20" : "bg-[var(--bg-tertiary)]")}>
                      {agent.icon || '\uD83E\uDD16'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className="text-sm font-medium text-[var(--text-primary)] truncate">{agent.name}</span>
                        {d.currentAgent?.id === agent.id && <span className="text-[9px] px-1 py-0.5 rounded bg-[var(--color-primary-500)]/30 text-[var(--color-primary-400)] flex-shrink-0">активен</span>}
                      </div>
                      {agent.description && <p className="text-[10px] text-[var(--text-tertiary)] truncate">{agent.description}</p>}
                    </div>
                  </button>
                  <div className="flex items-center gap-1">
                    <button onClick={(e) => { e.stopPropagation(); d.handleAgentSelect(agent); d.clearMessages(); d.setActivePanel('none'); }}

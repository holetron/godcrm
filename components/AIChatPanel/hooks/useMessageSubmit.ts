...
export function useMessageSubmit(params: UseMessageSubmitParams) {
  const {
    ...
    chatPartner, currentAgent, agentMode, thinkingEnabled, subAgents,
  ...
  const handleSubmit = useCallback(async (e?: FormEvent) => {
    ...
    trimmedInput = validateAndWrapMentions(trimmedInput, availableMentionUsers);
    ...
    try {
      if (chatPartner?.type === 'agent' && currentAgent) {
        ...
        await sendMessage(trimmedInput, filesToSend.length > 0 ? filesToSend : undefined, undefined, mentionsToSend, useAgentMode, undefined, subAgents.length > 0 ? subAgents : undefined, rowAtts);
        return;
      }
  ...
  }, [inputValue, attachments, messageBoundRows, chatPartner, currentAgent, agentMode, thinkingEnabled, subAgents, mentionedUsers, availableMentionUsers, availableSlashAgents, userConversationId, effectiveSpaceId, sendMessage, getOrCreateUserConversation, setInputValue, setAttachments, setMentionedUsers, setMessageBoundRows, setUserConversationId, setLocalError, sendUserMessageMutation, refetchUserMessages]);
